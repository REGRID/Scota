...
## 12.1 Catatan Tambahan dari Review Lanjutan

### A. Session/context "cabang mana yang sedang aktif" untuk Owner multi-tenant

Bab 6 sudah menjelaskan kalau Owner punya >1 tenant, sistem menampilkan "pemilih cabang". Yang perlu ditegaskan: **konteks tenant aktif harus selalu ditentukan dari `tenant-slug` di URL, bukan dari state/session global**.

```
❌ SALAH: menyimpan "active_tenant_id" di cookie/session tunggal
   → kalau Owner buka 2 tab browser untuk 2 cabang berbeda,
     keduanya akan tercampur karena berbagi satu session yang sama

✅ BENAR: setiap request menentukan konteks tenant dari
   `tenant-slug` di URL (/toko-kopi-abc/dashboard)
   → middleware validasi: apakah owner_id atau memberships user
     ini cocok dengan tenant-slug tersebut?
   → tiap tab browser otomatis independen, karena konteksnya
     menempel di URL masing-masing, bukan di satu session global
```

### B. Owner yang sudah punya tenant tapi membuka `/daftar` lagi

`/daftar` (Bab 7) seharusnya cuma untuk akun yang benar-benar baru (0 relasi ke tenant manapun). Kalau Owner yang **sudah** punya toko membuka `/daftar` lagi (misal lewat bookmark lama):

```
saat /daftar diakses, setelah Google login sukses:

  1. Cek: apakah user_id ini sudah punya ownedTenants atau staffMembership?

  JIKA YA:
    → JANGAN tampilkan error yang membingungkan
    → Redirect otomatis ke dashboard/pemilih cabang miliknya
    → Kalau dia Owner, boleh tampilkan opsi "Tambah Cabang Baru"
       di halaman tujuan (bukan lewat /daftar lagi)

  JIKA TIDAK (benar-benar baru):
    → Lanjutkan alur pembuatan tenant baru seperti biasa
```

### C. Staf membatalkan sendiri permintaan `PENDING_APPROVAL`

Bab 9 sudah punya alur Owner approve/reject, tapi staf sendiri juga perlu opsi membatalkan pengajuannya sendiri selagi masih menunggu (misal salah klik link, atau berubah pikiran) — supaya tidak "terjebak" menunggu keputusan Owner:

```
Staf di halaman "Menunggu Persetujuan Owner" klik "Batalkan Pengajuan":

  BEGIN TRANSACTION
  1. Copy baris memberships → INSERT ke `membership_history`
     (left_reason = 'CANCELLED_BY_STAFF', removed_by = NULL)
  2. DELETE baris dari `memberships`
  COMMIT

  → user_id staf ini langsung bebas dari constraint UNIQUE(user_id)
```
...
- [ ] Pastikan konteks tenant aktif SELALU diambil dari `tenant-slug` di URL, bukan dari session/cookie global (Bab 12.1.A)
- [ ] Redirect otomatis (bukan error) kalau Owner/staff yang sudah terdaftar membuka `/daftar` lagi (Bab 12.1.B)
- [ ] Tombol "Batalkan Pengajuan" untuk staf berstatus `PENDING_APPROVAL` (Bab 12.1.C)
