...
export function SubscriptionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-slate-950/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="relative w-full max-w-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl shadow-2xl overflow-hidden text-slate-900 dark:text-slate-100 flex flex-col max-h-[90vh]">
        {/* Header Bar */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base sm:text-lg font-bold tracking-tight text-slate-900 dark:text-white flex items-center gap-2">
                ...
                <span className="text-[11px] font-bold px-2 py-0.5 rounded-full bg-emerald-50 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/30 uppercase tracking-wide">
          ...
          {activeTab === "plans" && (
            <div className="space-y-6">
              ...
              <div className="flex justify-center">
                <div className="inline-flex items-center p-1 rounded-2xl bg-slate-100 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/60">
                  <button
                  ...
                  >
                    Tahunan
                    <span className="text-[10px] px-1.5 py-0.2 rounded-md bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-500/30 font-bold">
                      Hemat 17%
                    </span>
                  </button>
                </div>
                  ...
                  return (
                    <div
                      ...
                      className={`relative rounded-3xl p-5 border flex flex-col justify-between transition-all ${
                        isPro
                          ? "bg-white dark:bg-slate-800/80 border-2 border-emerald-500 shadow-xl dark:shadow-emerald-950/40 ring-1 ring-emerald-500/30"
                      ...
                      <div className="space-y-4">
                        ...
                        <div className="space-y-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                          {plan.features.map((feat, i) => (
                            <div key={i} className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-300 leading-snug">
                      ...
                      <div className="pt-6 space-y-2">
                        <button
                          ...
                          className={`w-full py-2.5 px-4 rounded-xl text-xs font-bold transition-all flex items-center justify-center gap-2 cursor-pointer ${
                            isCurrent
                              ...
                              : isPro
                              ? "bg-emerald-600 hover:bg-emerald-500 dark:bg-emerald-500 dark:hover:bg-emerald-400 text-white dark:text-slate-950 shadow-md active:scale-[0.98]"
                        ...
                        {!isCurrent && (
                          <button
                            ...
                            className="w-full py-2 px-3 rounded-xl bg-white dark:bg-slate-800/80 hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white border border-slate-200 dark:border-slate-700/60 text-[11px] font-semibold transition-colors flex items-center justify-center gap-1.5 cursor-pointer shadow-xs"
                          >
          ...
          {activeTab === "license" && (
            <div className="max-w-xl mx-auto space-y-6">
              <div className="text-center space-y-2">
                ...
                <h3 className="text-lg font-bold text-slate-900 dark:text-white">Aktivasi Kunci Lisensi</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              <form onSubmit={handleActivateLicense} className="space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Kode Lisensi / Voucher</label>
                  <input
                    type="text"
                    value={licenseInput}
                    onChange={(e) => setLicenseInput(e.target.value)}
                    placeholder="Contoh: NP-PRO-30D-XXXX-XXXX"
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 rounded-2xl px-4 py-3 text-sm font-mono text-emerald-600 dark:text-emerald-400 uppercase tracking-widest placeholder:tracking-normal placeholder:font-sans placeholder:text-slate-400 dark:placeholder:text-slate-600 outline-none"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-2xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 disabled:opacity-50 text-white font-bold text-sm transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer active:scale-[0.98]"
                >
                  {isActivating ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
              ...
              <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/40 border border-slate-200 dark:border-slate-800 space-y-2 text-xs text-slate-600 dark:text-slate-400">
                <h4 className="font-bold text-slate-800 dark:text-slate-200">Belum memiliki kunci lisensi?</h4>
                <p>
                  Pilih paket di tab <strong>Pilihan Paket</strong> atau hubungi tim billing untuk mendapatkan kode aktivasi resmi usaha Anda.
                </p>
              </div>
            </div>
          )}
            ...
            <form onSubmit={handleSaveProfile} className="max-w-2xl mx-auto space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <Building2 className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Nama Usaha / Toko / Perusahaan <span className="text-rose-500">*</span>
                  </label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 focus:border-emerald-500 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-white outline-none"
                  />
                  <p className="text-[11px] text-slate-500 dark:text-slate-400">Nama ini akan dicetak di header resmi laporan PDF & Excel.</p>
                </div>

                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300">Slogan / Tagline Usaha</label>
                  <input
                    type="text"
                    value={tagline}
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <Phone className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> No. Telepon / WhatsApp Usaha
                  </label>
                  <input
                    type="text"
                    value={phone}
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <MapPin className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Alamat Lengkap Kantor / Toko
                  </label>
                  <input
                    type="text"
                    value={address}
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  <label className="text-xs font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
                    <FileText className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" /> Catatan Footer Laporan PDF
                  </label>
                  <textarea
                    rows={2}
                    value={invoiceFooter}
                    ...
                    className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-700 focus:border-emerald-500 rounded-xl px-3.5 py-2.5 text-sm text-slate-900 dark:text-white outline-none resize-none"
                  />
                </div>
              </div>
              ...
              <div className="pt-3">
                <button
                  ...
                  className="w-full py-3 px-4 rounded-xl bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 disabled:opacity-50 text-white font-bold text-sm transition-all shadow-md flex items-center justify-center gap-2 cursor-pointer active:scale-[0.98]"
                >
                  {isSavingProfile ? (
                    <>
                      <Loader2 className="w-4 h-4 animate-spin" />
