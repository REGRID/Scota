          ...
          <div className="mt-2.5 flex flex-wrap gap-2">
            {item.actions.map((action) => (
              <button
                ...
                className={cn(
                  "inline-flex h-8 items-center rounded-[9px] px-3 text-[12.5px] font-medium transition-colors focus-visible:ring-2 focus-visible:ring-neutral-900/25 focus-visible:ring-offset-2 focus-visible:outline-none dark:focus-visible:ring-white/30 dark:focus-visible:ring-offset-neutral-900 cursor-pointer",
                  SQUIRCLE,
                  action.tone === "primary"
  ...
  function NotificationPanel(
    ...
    return (
      <div
        ...
        className={cn(
          "w-full max-w-[440px] overflow-hidden rounded-[20px] border border-black/[0.08] bg-white text-neutral-900 antialiased shadow-2xl dark:border-white/[0.12] dark:bg-neutral-900 dark:text-neutral-50",
          SQUIRCLE,
          className,
        )}
      >
        ...
        <div className="flex items-center justify-between gap-3 px-4 pt-3.5 pb-0.5">
          <div className="flex items-center gap-2">
            <h2 className="m-0 text-[15px] font-bold tracking-[-0.01em]">Notifikasi</h2>
            {unread > 0 && (
              <span className="rounded-full bg-blue-500/15 px-2 py-0.5 text-[11px] font-bold text-blue-600 dark:text-blue-400">
                {unread} Baru
              </span>
            )}
          </div>
          <button
            ...
            disabled={unread === 0}
            className="inline-flex items-center gap-1.5 rounded-[8px] px-2 py-1 text-[12px] font-medium text-neutral-600 transition-colors hover:text-neutral-900 hover:bg-neutral-100 disabled:pointer-events-none disabled:opacity-40 cursor-pointer dark:text-neutral-300 dark:hover:text-neutral-50 dark:hover:bg-neutral-800"
          >
        ...
        <div className="flex items-center gap-1 border-b border-black/[0.06] px-3 dark:border-white/[0.08]">
            ...
            {TABS.map((t) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative shrink-0 px-2.5 py-2.5 text-[12.5px] font-medium transition-colors cursor-pointer",
                    active
                ...
                >
                  ...
                  {active ? (
                    <motion.span
                      ...
                      className="absolute inset-x-1.5 -bottom-px h-[2.5px] rounded-full bg-neutral-900 dark:bg-neutral-50"
                    />
                  ) : null}
                </button>
              );
          ...
          {onSettings && (
            <button
              ...
              className={cn(
                "grid h-7 w-7 shrink-0 place-items-center rounded-[8px] text-neutral-400 transition-colors hover:bg-black/[0.05] hover:text-neutral-700 cursor-pointer dark:hover:bg-white/[0.08] dark:hover:text-neutral-100",
                SQUIRCLE,
              )}
            >
        ...
        >
              ...
              >
                <Row
                  ...
                  onAction={(actionId) => {
                    ...
                    setResolved((r) => ({
                      ...r,
                      [item.id]: action?.resolved ?? `Tindakan: ${action?.label ?? actionId}`,
                    }));
                    patch(item.id, { unread: false });
          ...
          {shown.length === 0 ? (
            <div className="flex flex-col items-center gap-2.5 px-6 py-12 text-center">
              <span
                className={cn(
                  "grid h-11 w-11 place-items-center rounded-[14px] bg-neutral-100 text-neutral-400 dark:bg-white/[0.06] dark:text-neutral-500",
                  SQUIRCLE,
                )}
              >
                <Bell className="h-5 w-5" />
              </span>
              <p className="m-0 text-[13px] font-semibold text-neutral-800 dark:text-neutral-200">
                {tab === "archived" ? "Tidak ada arsip notifikasi" : "Semua sudah terbaca"}
              </p>
              <p className="m-0 text-[12px] text-neutral-500 dark:text-neutral-400 max-w-[260px]">
                {tab === "archived"
                  ? "Notifikasi yang Anda arsipkan akan muncul di sini."
                  : "Aktivitas transaksi, persetujuan nota, dan alert baru akan muncul di sini."}
              </p>
            </div>
          ) : null}
        </div>
