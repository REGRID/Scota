...
-- 20. Dynamic Roles, Granular Permissions, Multi-Tenant Grants & Tenant Features (Spec 4)
...
-- Insert standard fixed permissions dictionary
...
-- Idempotent column alterations
ALTER TABLE memberships ADD COLUMN IF NOT EXISTS "roleId" UUID REFERENCES roles(id) ON DELETE SET NULL;
ALTER TABLE membership_history ADD COLUMN IF NOT EXISTS "roleId" UUID REFERENCES roles(id) ON DELETE SET NULL;
ALTER TABLE membership_history ADD COLUMN IF NOT EXISTS "sourceTable" VARCHAR(20) NOT NULL DEFAULT 'MEMBERSHIP';
ALTER TABLE invite_links ADD COLUMN IF NOT EXISTS "roleId" UUID REFERENCES roles(id) ON DELETE SET NULL;
