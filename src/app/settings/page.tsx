...
function SettingsContent() {
  ...
  const handleOpenEditRole = (role: any) => {
    ...
    setEditRoleRequiresApproval(!!role.requiresApproval)
  ...
  const handleSaveRole = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(data.message || "Peran berhasil diperbarui")
      setEditingRole(null)
      fetchRoles()
    } catch (err: any) {
  ...
  const handleDeleteRole = async (roleId: string, roleName: string) => {
    if (!confirm(`Apakah Anda yakin ingin menghapus peran "${roleName}"?`)) return
    try {
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || "Gagal menghapus peran")
      toast.success(data.message || "Peran berhasil dihapus")
      fetchRoles()
    } catch (err: any) {
  ...
  const handleSelfResign = async () => {
    if (!confirm("Apakah Anda yakin ingin mengundurkan diri dari toko ini? Akses Anda ke toko ini akan ditutup.")) return
    try {
      setIsResigning(true)
      const headers = await getAuthenticatedHeaders()
      ...
      const data = await res.json()
      if (!res.ok) throw new Error(data.error || "Gagal mengundurkan diri")
      toast.success(data.message || "Berhasil mengundurkan diri")
      router.push("/login")
    } catch (err: any) {
      toast.error(err.message || "Gagal mengundurkan diri")
        ...
        <main className="flex-1 min-w-0">
          ...
          {activeTab === "users" && (
            <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 space-y-6 shadow-xs">
                            ...
                            <td className="p-3">
                              {isProtected ? (
                                ...
                                >
                                  {dynamicRoles.length > 0 ? (
                                    dynamicRoles
                                      .filter((r) => r.name?.toUpperCase() !== "OWNER" && r.name?.toUpperCase() !== "SUPERADMIN")
                                      .map((r) => (
              ...
              {/* Staff Self-Resignation Option (Only for non-owners) */}
              {currentUserRole.toUpperCase() !== "OWNER" && (
                <div className="p-4 rounded-xl bg-rose-50/50 dark:bg-rose-950/20 border border-rose-200/60 dark:border-rose-900/40 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                  <div>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <h4 className="text-xs font-black text-slate-900 dark:text-white flex items-center gap-1.5 uppercase tracking-wider">
                      ...
                      <span>Hak Akses & Manajemen Peran</span>
                    </h4>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">
                      Daftar peran aktif dan hak izin (permissions) yang berlaku di sistem toko Anda.
                    </p>
                  </div>
                  {currentUserRole.toUpperCase() === "OWNER" && tenantFeatures.custom_roles && (
                    <button
                      type="button"
                      onClick={() => setShowCreateRoleModal(true)}
              ...
              {/* Modal: Edit Peran & Atur Izin (RBAC) */}
              {editingRole && (
                <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-xs flex items-center justify-center p-4">
                  <div className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-5 sm:p-6 w-full max-w-lg shadow-xl space-y-4 animate-in fade-in zoom-in-95">
                    <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
                    ...
                    <form onSubmit={handleSaveRole} className="space-y-4 text-xs">
                      <div className="space-y-1">
                        ...
                        {editingRole.isSystemDefault && (
                          <p className="text-[10px] text-slate-400">
                            *Nama peran standar sistem tidak dapat diganti, namun hak izinnya dapat disesuaikan.
                          </p>
                        )}
                      </div>
